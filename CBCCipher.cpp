...
namespace cppCrypto_0_0_0 {
    ...
    CBCCipher::CBCCipher( const octet_type *iv, BlockCipher &cipher ) :
            ...
            underlying( cipher ) {
        std::memcpy( old_ciphertext.get( ), iv, underlying.block_size( ));
    }
    ...
    void CBCCipher::encrypt( octet_type *block ) {
        ...
        std::memcpy( old_ciphertext.get( ), block, underlying.block_size( ));
    }


    void CBCCipher::decrypt( octet_type *block ) {
        // Save the ciphertext aside for later.
        std::unique_ptr < octet_type[] > temp{ new octet_type[underlying.block_size( )] };
        std::memcpy( temp.get( ), block, underlying.block_size( ));
        ...
        std::memcpy( old_ciphertext.get( ), temp.get( ), underlying.block_size( ));
    }
